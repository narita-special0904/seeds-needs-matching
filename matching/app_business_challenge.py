def render_business_challenge():
    ...
    class NeedsSeedsSearchUI:
        def __init__(self):
            ...
            self.gremlin_client = client.Client(
                ...
                f"/colls/{os.getenv('AZURE_COSMOSDB_GREMLIN_CONTAINER_BUSINESS_CHALLENGE')}",
            ...
            # 起点ファイルノードの「経営課題」メタタイプノードから、他の「経営課題」メタタイプノードへ結合させる本数と、潜在ニーズ取得数
            self.n_limit = 2
        ...
        # ------------------------------------------------------------------
        def traverse_related_business_challenge(self, gremlin_id: str, origin_filename: str = "", is_primary: bool = True, other_origin_ids: List[str] = None) -> Tuple[List[Dict], Dict]:
            """起点ファイルから business_challenge 関連をたどり、可視化グラフを組み立てる"""
            try:
                ...
                graph = {
                    ...
                    "nodes": [],
                    "edges": [],
                    "business_challenge_nodes": {},  # File ID → business_challenge graph node ID
                }
                ...
                # 起点 File → business_challenge → business_challenge_nodes → File（limit N）
                simple_q = f"""
                g.V('{gremlin_id}')
                .out('has_metatype').has('type','business_challenge')
                .outE('related_business_challenge').as('e')
                .inV()
                ...
                # 起点 business_challenge → 他 File の business_challenge へ強制2本エッジ
                self._link_business_challenge_edges(graph)
                        ...
                        graph["nodes"].append(
                            {
                                ...
                                "type": "metatype",
                                "color": "#a96bff" if mt_type == "business_challenge" else "#2ca02c",
                                "size": 20,
                                "hidden": False,  # MetaTypeノードは常に表示
                            }
                        )
                        # business_challenge ノードを控える
                        if mt_type == "business_challenge":
                            graph["business_challenge_nodes"][file_id] = mt_id
        ...
        # ----------------------------------------------------------------------------
        # business_challenge ノード間に related_business_challenge エッジN本を付与
        # ----------------------------------------------------------------------------
        def _link_business_challenge_edges(self, graph: Dict):
            try:
                origin_fid = graph.get("origin_file_id")
                tp = graph.get("business_challenge_nodes", {})
                    ...
                    graph["edges"].append(
                        {
                            "from": origin_tp_id,
                            "to": dest,
                            "label": "related_business_challenge",
                            "color": "#0066ff",
                            "width": 4,
        ...
        # ------------------------------------------------------------------
        # 「business_challenge」MetaType間のエッジ追加
        # ------------------------------------------------------------------
        def _add_business_challenge_edges(self, graph_data: Dict):
            try:
                ...
                for mt_id in mt_ids:
                    ...
                    for r in self.gremlin_client.submit(rel_query).all().result():
                        ...
                        if dest_id in mt_ids and not any(
                            e["from"] == mt_id and e["to"] == dest_id and e["label"] == "related_business_challenge"
                ...
                logger.error(f"related_business_challenge Edge add error: {e}")
    ...
    def main():
        ...
        # セッション状態
        for k, v in [
            ("business_challenge_search_results", []),
            ("business_challenge_related_files", []),
            ("business_challenge_related_files_1", []),
            ("business_challenge_related_files_2", []),
            ("business_challenge_graph_data", {"nodes": [], "edges": []}),
            ("business_challenge_analysis_result", ""),
            ("business_challenge_pdf_bytes", ""),
            ("business_challenge_table_data", []),
            ("business_challenge_table_data_1", []),
            ("business_challenge_table_data_2", []),
        ]:
        ...
        # 質問入力欄（画面切り替え時の入力値保持）
        query_key = "query_key_business_challenge"
        ...
        if st.button("🔍 解析開始", type="primary") and query:
            with st.spinner("処理中...", show_time=True):
                # AI Search結果
                st.session_state.business_challenge_search_results = ui.search_documents(query)

                if st.session_state.business_challenge_search_results:
                    top    = st.session_state.business_challenge_search_results[0]
                    second = st.session_state.business_challenge_search_results[1]
                    st.info(f"Matching TOP1： {top['filename']}")
                    st.info(f"Matching TOP2： {second['filename']}")

                    # Gremlinグラフをトラバーサルした結果をsession_stateに保存
                    # TOP1
                    (st.session_state.business_challenge_related_files_1, st.session_state.business_challenge_graph_data) = ui.traverse_related_business_challenge(
                        top["gremlin_id"], 
                        top["filename"], 
                        is_primary=True,
                    ...
                    # TOP2
                    (related_file_2, graph_data_2) = ui.traverse_related_business_challenge(
                        second["gremlin_id"], 
                        second["filename"], 
                        is_primary=False,
                        other_origin_ids=[top["gremlin_id"]]  # TOP1のIDを渡す
                    )
                    st.session_state.business_challenge_related_files_2 = related_file_2

                    #-----------------------------------------------------------------
                    # TOP2の間接ファイルノードで色の上書き防止
                    #-----------------------------------------------------------------
                    st.session_state.business_challenge_graph_data["nodes"] += graph_data_2["nodes"]
                    st.session_state.business_challenge_graph_data["edges"] += graph_data_2["edges"]


                    if st.session_state.business_challenge_related_files_1 or st.session_state.business_challenge_related_files_2:
                        st.success(f"関連ファイルを{len(st.session_state.business_challenge_related_files_1)+len(st.session_state.business_challenge_related_files_2)}件発見しました")
                    else:
                    ...
                    # 顕在ニーズ
                    for res in st.session_state.business_challenge_search_results[:2]:
                    ...
                    # 潜在ニーズ
                    # TOP1の関連情報
                    for rel in st.session_state.business_challenge_related_files_1:
                    ...
                    # TOP2の関連情報
                    for rel in st.session_state.business_challenge_related_files_2:
                        summary = ui.generate_proposal_summary(rel, query)
                        table_data_2.append(
                            {
                                "課題種別": "間接課題",
                    ...
                    if table_data_1:
                        try:
                            # TOP1
                            q = f"g.V('{st.session_state.business_challenge_search_results[0]['gremlin_id']}').values('blob_url')"
                    ...
                    if table_data_2:
                        try:
                            # TOP2
                            q = f"g.V('{st.session_state.business_challenge_search_results[1]['gremlin_id']}').values('blob_url')"
                            r = ui.gremlin_client.submit(q).all().result()
                            if r:
                    ...
                    # セッション保存
                    st.session_state.business_challenge_table_data_1 = table_data_1
                    st.session_state.business_challenge_table_data_2 = table_data_2
                    st.session_state.business_challenge_table_data   = table_data_1 + table_data_2

                    st.session_state.business_challenge_related_files = st.session_state.business_challenge_related_files_1 + st.session_state.business_challenge_related_files_2 


                    # --- 総合解析呼び出し（table_data含む） ---
                    with st.spinner("総合解析中...", show_time=True):
                        st.session_state.business_challenge_analysis_result = ui.generate_comprehensive_analysis(
                            query,
                            st.session_state.business_challenge_search_results,
                            st.session_state.business_challenge_related_files,
                            st.session_state.business_challenge_table_data,  # ←ここで画面の表内容を反映
                        )
                else:
                    st.error("検索結果が見つかりませんでした")

        # ───────── 提案候補テーブル ─────────
        if st.session_state.business_challenge_search_results or st.session_state.business_challenge_related_files:
            st.subheader("📋 技術展開可能性")
            st.caption("マッチングスコアは相対的なものです")
            table_data = st.session_state.get("business_challenge_table_data", [])
            ...
            if table_data:
                ...
                .tech-table th {
                    business_challenge-color: #f2f2f2;
                    font-weight: bold;
                }
        ...
        with tab1:
            if st.session_state.business_challenge_analysis_result:
                st.markdown(st.session_state.business_challenge_analysis_result)

                # PDFダウンロード機能
                BASE_DIR = os.path.dirname(os.path.abspath(__file__))
                FONT_PATH = os.path.join(BASE_DIR, "fonts", "fonts-japanese-gothic.ttf")
                pdf_bytes = make_pdf_jp(st.session_state.business_challenge_analysis_result, font_path=FONT_PATH)
                st.download_button(
        ...
        with tab2:
            if st.session_state.business_challenge_graph_data["nodes"]:
                html = ui.visualize_graph(st.session_state.business_challenge_graph_data)
                # Debug
